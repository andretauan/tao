# CLAUDE.md — Instruções Universais para Agentes | {{PROJECT_NAME}}

> **LEIA ESTE ARQUIVO INTEIRO ANTES DE QUALQUER AÇÃO.**
> Todo agente que atue neste projeto, por menor que seja a tarefa, deve:
> 1. Ler `CLAUDE.md` (este arquivo)
> 2. Ler `CONTEXT.md` (estado atual)
> 3. Consultar `CHANGELOG.md` (últimas 5 entradas)
> 4. Verificar `.github/skills/INDEX.md` se existir — ler skills aplicáveis
> Só então: executar.
...
**Configuração:** `tao.config.json` é a **fonte única de verdade** para configuração do projeto.
Contém: caminhos de código-fonte, comandos de lint, modelos, política de branch e todas as configurações de runtime.
Agentes DEVEM ler `tao.config.json` para resolver valores específicos do projeto (stack, caminhos, comandos).

| Arquivo | Propósito |
|---------|-----------|
| `tao.config.json` | Configuração do projeto — caminhos, modelos, lint, branch, escopos |
| `CONTEXT.md` | Estado atual — fase ativa, decisões, arquivos tocados |
| `CHANGELOG.md` | Histórico — o que mudou, quando, por quem |
| `phases/` | Docs de fase — planos, status, tarefas, progresso |
...
| **Shen-Architect** | 深 | Opus | Arquiteto — invocável pelo usuário, acesso direto fora do loop |
...
Comandos de lint são definidos em `tao.config.json` → `lint_commands`.
...
Após cada arquivo editado ou criado: atualizar `CONTEXT.md` seção "Arquivos Tocados (sessão)."
Nunca deixar TODO ou FIXME sem registrar em `CONTEXT.md` seção "Pendências Abertas."
...
| Lock | Regra |
|------|-------|
| **LOCK 1 — ESCOPO** | Modificar apenas arquivos-fonte do projeto. NUNCA modificar: `CLAUDE.md`, `.github/workflows/`, `vendor/`, `node_modules/`, `venv/`, `.env`, `tao.config.json` (sem aprovação explícita). |
| **LOCK 2 — BRANCH** | Somente `dev` (ou conforme definido em `tao.config.json` → `branch`). NUNCA `git push origin main`, `git push --force`, `git reset --hard`. |
...
```
1. Brainstorm → DISCOVERY.md + DECISIONS.md  (somente Opus — agente @Wu)
2. Sintetizar → BRIEF.md                      (somente Opus — maturity gate ≥ 5/7)
3. Planejar   → PLAN.md                       (somente Opus — @Wu ou @Shen-Architect)
...
**Escopos:** definidos por projeto em `tao.config.json` → `commit_scopes`. Exemplos comuns: `api`, `auth`, `db`, `ui`, `core`, `deploy`.
...
Todo `CONTEXT.md` DEVE conter estes campos:
...
> São populados durante a instalação com base na sua stack.
> Referência: `tao.config.json` → `stack` para convenções por linguagem.
...
- [ ] Ler `CLAUDE.md` (este arquivo)
- [ ] Ler `CONTEXT.md` (estado atual)
- [ ] Consultar `CHANGELOG.md` (últimas 5 entradas)
- [ ] Verificar `.github/skills/INDEX.md` (se existir)

### Após qualquer sessão
- [ ] Timestamp obtido via `date '+%Y-%m-%d %H:%M'` (R4)
- [ ] `CONTEXT.md` atualizado com timestamp
- [ ] Entrada registrada no `CHANGELOG.md` com HH:MM
