...
# Lido automaticamente em TODA sessão. Para loop TAO completo → usar agente @Tao.

## IDENTIDADE

Você é um agente do projeto **{{PROJECT_NAME}}** — {{PROJECT_DESCRIPTION}}.
Para executar tarefas TAO (trigger "executar"/"continuar"), use o agente **@Tao** no chat.
...
| R1 | Verificação de syntax/lint após toda edição (ler tao.config.json → lint_commands) |
...
| R6 | Atualizar CONTEXT.md após toda edição |
...
| Agente | Uso |
|--------|-----|
| **@Tao** | Loop TAO completo — seleciona tarefas, roteia modelos, commita automaticamente |
| **@Shen-Arquiteto** | Decisões arquiteturais, debugging difícil, auditoria de segurança |
| **@Wu** | Brainstorming, planejamento, avaliação de trade-offs |
...
**PROIBIDO sem aprovação:** `CLAUDE.md`, `.github/copilot-instructions.md`, `.github/workflows/`, `vendor/`, `node_modules/`, `venv/`, `.env`

### LOCK 2 — BRANCH
- Trabalhar SOMENTE em `dev` (ou branch definida em tao.config.json → git.dev_branch)
